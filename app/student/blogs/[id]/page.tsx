...
export default function StudentBlogDetailPage() {
    ...
    async function loadBlog() {
        ...
        const res = await fetch(`/api/student/blogs/${blogId}`);
    ...
    async function loadCommentUsers() {
        const [meRes, tutorsRes] = await Promise.all([
            fetch("/api/me"),
        ...
        const rawTutors: unknown[] = Array.isArray(tutorsData) ? tutorsData : tutorsData?.content ?? [];

        rawTutors.forEach((item) => {
            ...
            if (typeof tutor.id !== "string") return;
    ...
    async function onSubmitComment(e: FormEvent) {
        ...
        try {
            const res = await fetch(`/api/student/blogs/${blogId}/comments`, {
                method: "POST",
                headers: {
    ...
    async function saveEdit(commentId: string) {
        ...
        try {
            const res = await fetch(`/api/student/blogs/comments/${commentId}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                },
        ...
        <div className="mx-auto max-w-4xl space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <Button asChild variant="secondary">
                    <Link href="/student/blogs">Back</Link>
                </Button>
            </div>
