...
const navItems = [
    ...
    { href: "/student/tutor", label: "Tutor Information", icon: UserRound },
...
export default function StudentLayout({ children }: { children: React.ReactNode }) {
    ...
    const currentLabel = useMemo(() => {
        const current = navItems.find((item) =>
            isActivePath(pathname, item.href, "/student")
        );
        return current?.label ?? "Student Dashboard";
    }, [pathname]);
    ...
    const sidebarContent = (
        <>
            ...
            <div className="mb-8 flex flex-col items-center gap-2 rounded-2xl bg-white/5 px-4 py-5">
                ...
                <div className="text-sm font-medium opacity-90">Student</div>
                <div className="text-xs text-white/60">Learning Portal</div>
            </div>

            <nav className="space-y-1">
                {navItems.map((item) => {
                    const active = isActivePath(pathname, item.href, "/student");
